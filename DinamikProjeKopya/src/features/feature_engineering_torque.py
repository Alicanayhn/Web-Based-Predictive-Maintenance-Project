...
def drop_failure(dataframe):
    dataframe.drop(["Failure Type"],axis=1,inplace=True)
...
def scaler(dataframe):
    ...
    sayisal_degiskenler = [col for col in sayisal_degiskenler if col != "Torque [Nm]"]
    ss = StandardScaler()
...
def make_feature_engineering_torque(dataframe):
    set_options()
    ...
    dataframe = drop_failure(dataframe)
