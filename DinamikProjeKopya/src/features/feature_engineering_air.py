...
def drop_timestamp(dataframe):
    ...
    dataframe.drop("Air_Leak_Failure",axis = 1,inplace = True)
...
def scaler(dataframe):
    ...
    kategorik_degiskenler = [col for col in kategorik_degiskenler if col != "Air_Leak_Failure"]
...
def make_feature_engineering_air(dataframe):
    set_options()
